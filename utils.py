...
def image_grid(images: List[Image.Image], col: int, gap: int = 10) -> Image.Image:
...
def image_grid_with_name(
    images, col_names, row_names, col, row_gap=10, col_gap=10, font_size=20
):
    row = math.ceil(len(images) / col)
    cell_width = images[0].width
    cell_height = images[0].height

    # Calculate total grid width and height with gaps
    grid_width = col * cell_width + (col - 1) * col_gap
    grid_height = row * cell_height + (row - 1) * row_gap

    # Define the size of the text margin for row and column names
    text_margin = max(font_size, 50)  # Adjust this as needed

    # Create a new image to hold the grid with additional space for text
    total_width = grid_width + text_margin
    total_height = grid_height + text_margin
    ...
    for i, image in enumerate(images):
        ...
        # Calculate the coordinates to paste the image into the grid
        paste_x = col_idx * (cell_width + col_gap) + text_margin
        paste_y = row_idx * (cell_height + row_gap) + text_margin
...
def preprocess_canny(image, preprocess_image_size, output_image_size):
    ...
    processed_image = canny_detector(
        image,
        detect_resolution=preprocess_image_size,
        image_resolution=output_image_size,
    )
...
def preprocess_sketch_canny(image, preprocess_image_size, output_image_size):
    image, _ = resize_image_with_pad(np.array(image), preprocess_image_size)
    image = Image.fromarray(image)
    sketch_processed_image = pidi_detector(
        image,
        apply_filter=True,
        detect_resolution=preprocess_image_size,
        image_resolution=output_image_size,
    )
    canny_processed_image = canny_detector(
        image,
        detect_resolution=preprocess_image_size,
        image_resolution=output_image_size,
        low_threshold=150,
